  ...
  if (isProductLoading) {
    return (
      <div className="min-h-screen bg-white pb-24 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-zinc-500 font-bold uppercase tracking-widest text-sm">Loading Product Data...</p>
      </div>
    );
  }
        ...
        <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-10">
          ...
          <div className="sticky top-0 z-10 bg-white/80 backdrop-blur-md py-4 border-b border-zinc-100 flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-black text-zinc-900 tracking-tight uppercase">Edit Product</h1>
                <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-[0.3em]">Update Catalog Details</p>
              </div>
            </div>
            <button 
              type="submit" 
              ...
              <div className="bg-zinc-50 p-8 rounded-[40px] border border-zinc-100 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="col-span-2 space-y-1">
                    ...
                    <input {...register("name", { required: true })} className="w-full p-4 border rounded-2xl focus:ring-2 focus:ring-[#006044] outline-none font-bold text-lg bg-white" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Base Selling Price *</label>
                    <input {...register("price", { required: true })} type="number" step="0.01" className="w-full p-4 border rounded-2xl outline-none font-black text-xl bg-white focus:ring-2 focus:ring-[#006044]" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">MRP (Old Price)</label>
                    <input {...register("oldPrice")} type="number" step="0.01" className="w-full p-4 border rounded-2xl outline-none text-zinc-400 font-bold bg-white focus:ring-2 focus:ring-[#006044]" />
                  </div>
                </div>

                <div className="space-y-1">
                  <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Product Description</label>
                  <textarea {...register("description")} rows={5} className="w-full p-5 border rounded-3xl outline-none focus:ring-2 focus:ring-[#006044] font-medium leading-relaxed bg-white" />
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {attrFields.map((field, index) => (
                    <div key={field.id} className="flex items-center bg-white p-2 rounded-2xl border">
                      <input {...register(`attributes.${index}.name` as any)} placeholder="Key" className="w-1/2 p-2 outline-none text-xs font-black uppercase tracking-tighter" />
