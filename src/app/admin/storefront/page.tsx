...
export default function StorefrontBuilderPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      return adminService.updateThemeConfig(storeId, { sectionsOrder: store.sections });
    },
    onSuccess: () => {
  ...
  if (isLoading) return <div className="flex h-screen items-center justify-center"><Loader2 className="animate-spin text-[#006044] w-8 h-8"/></div>;
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-gray-100">
      ...
      <header className="flex-shrink-0 h-16 bg-white border-b px-6 flex items-center justify-between z-10 shadow-sm">
        ...
        >
          {saveMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save size={16} />}
          Publish Changes
        </button>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-80 bg-white border-r flex flex-col h-full z-10 overflow-y-auto p-5 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
          <h2 className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3">Add Block</h2>
          <div className="grid grid-cols-2 gap-2 mb-8">
            {AVAILABLE_SECTIONS.map((block) => (
              <button key={block.type} onClick={() => store.addSection(block.type as any)} className="flex flex-col items-center justify-center p-3 border border-gray-100 rounded-xl hover:border-[#006044] hover:bg-[#006044]/5 hover:shadow-sm text-xs font-bold text-gray-700 transition-all text-center h-full">
                <Plus size={16} className="mb-1.5 text-gray-400" /> {block.label}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] font-bold bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">{store.sections.length}</span>
          </div>
