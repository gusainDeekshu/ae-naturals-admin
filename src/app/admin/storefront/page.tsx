...
import { 
  LayoutTemplate, Plus, Save, ArrowUp, ArrowDown, 
  Edit2, Trash2, Eye, EyeOff, Store, Loader2
} from "lucide-react";
...
const AVAILABLE_SECTIONS = [
  { type: "HERO", label: "Hero Banner Slider", desc: "Top of page dynamic slider." },
  { type: "TRUST_BADGES", label: "Trust Badges Ticker", desc: "Scrolling validation bar." },
  { type: "CATEGORIES", label: "Category Grid", desc: "Visual category navigation." },
  { type: "PRODUCT_CAROUSEL", label: "Product Carousel", desc: "Swipeable product row." },
  { type: "PROMO_BANNER", label: "Promotional Banner", desc: "Full-width image breakout." },
  { type: "BRAND_STORY", label: "Brand Story Block", desc: "Editorial text and image." },
  { type: "BLOG_SECTION", label: "Journal / Blog", desc: "Latest articles feed." },
];
...
export default function StorefrontBuilderPage() {
  ...
  // Fetch Store ID & Current Config
  const { data: homeData, isLoading } = useQuery({
    queryKey: ["store-home-config"],
    queryFn: async () => {
      const stores: any = await adminService.getStores();
      const currentStore = stores?.[0]; 
      if (currentStore) setStoreId(currentStore.id);
      
      const payload: any = await adminService.getHomepageData();
      return payload;
    },
  });
  ...
  useEffect(() => {
    if (homeData?.config?.sectionsOrder) {
      setSections(homeData.config.sectionsOrder);
    }
  }, [homeData]);

  // Save Mutation
  const saveMutation = useMutation({
    mutationFn: (newSections: ThemeSection[]) => 
      adminService.updateThemeConfig(storeId, { sectionsOrder: newSections }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["store-home-config"] });
      alert("Storefront layout saved successfully!");
    },
    onError: () => {
      alert("Failed to save layout. Please check console.");
    }
  });
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-gray-400 gap-4">
        ...
        <p className="font-medium tracking-widest uppercase text-sm">Loading Storefront Config...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1.5 font-medium">
            Design your homepage layout. Changes instantly reflect on the production storefront.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* LEFT COLUMN: The Live Layout Canvas */}
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-bold text-gray-900 tracking-tight">Active Layout Sequence</h2>
            <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">{sections.length} Sections</span>
          </div>
          ...
          {sections.length === 0 && (
            <div className="p-12 text-center border-2 border-dashed border-gray-200 rounded-2xl text-gray-400 bg-gray-50">
              ...
              <p className="text-sm mt-1">Build your page by adding blocks from the toolbox.</p>
            </div>
          )}
              ...
              >
                <div className="flex items-center gap-5">
                  <div className="flex flex-col gap-1 bg-gray-50 p-1 rounded-lg border border-gray-100">
                    <button onClick={() => moveSection(index, 'UP')} disabled={index === 0} className="text-gray-400 hover:text-gray-900 disabled:opacity-20 p-1 transition-colors"><ArrowUp size={16} strokeWidth={3} /></button>
                    <button onClick={() => moveSection(index, 'DOWN')} disabled={index === sections.length - 1} className="text-gray-400 hover:text-gray-900 disabled:opacity-20 p-1 transition-colors"><ArrowDown size={16} strokeWidth={3} /></button>
                  </div>
                  <div>
                    <h3 className="font-bold text-gray-900 tracking-wide text-sm">{section.type.replace('_', ' ')}</h3>
                    <p className="text-[11px] text-gray-500 font-medium uppercase tracking-widest mt-1">
                      {section.settings?.title || section.settings?.dataSource || "Default Theme Configuration"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button 
                    onClick={() => { setEditingIndex(index); setIsModalOpen(true); }}
        ...
        <div className="lg:col-span-4 space-y-4">
          <h2 className="text-lg font-bold text-gray-900 tracking-tight mb-2">Available Blocks</h2>
            ...
            {AVAILABLE_SECTIONS.map((block) => (
              ...
              >
                ...
                <div className="bg-gray-50 p-2 rounded-lg group-hover:bg-[#006044]/10 transition-colors">
                  <Plus size={18} className="text-gray-400 group-hover:text-[#006044]" />
                </div>
              </button>
            ))}
          </div>
